...
def generate_opencode_config(project_root: Path, absolute: bool = False) -> dict:
    command = shutil.which("brain_ds")
    if command is None:
        raise RuntimeError("brain_ds not found on PATH. Install via `pip install -e .` or add to PATH.")
    ...
    return {
        "mcp": {
            "brain_ds": {
                "type": "local",
                "command": [command, "mcp", "--project-root", root_value],
                "environment": {"BRAIN_DS_PROJECT_ROOT": root_value},
                "enabled": True,
            }
        }
    }
