...
5. **verify** — run the compliance gate on all `.elicit/` artifacts; write the `verify-<slug>-<date>.md` report. A passing gate is required before archive. Then compute the deterministic `SemanticReport` (`build_semantic_report`), persist/reference it as engram key `org/<slug>/verify/semantic-<ISO-date>`, and delegate `brainds-semantic-verifier` with the org slug, graph id, brd node id, and the SemanticReport reference. Relay its tiered advisory findings to the user — advisory only, never blocks archive.
