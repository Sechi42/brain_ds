...
| **Tokens (CANONICAL source)** | `brain_ds/ui/static/tokens.css` (`_tokens.css` is a forwarding shim) |
...
### 1. Token discipline
- **Never** hardcode hex colors in templates. Use `var(--*)` tokens.
- The CANONICAL token source is `brain_ds/ui/static/tokens.css` (Phase D.1, modern-design-tokens). It is inlined at render time by `template_renderer.py`.
- `brain_ds/ui/theme.py` `THEME_TOKENS` is DATA for contrast tooling + tests only — it is NOT a CSS emitter. Do not reintroduce one.
- If a new token is needed, add it to `static/tokens.css` (both `:root` and `[data-theme="light"]` when it is a color) and keep `theme.py` mirrors in sync only when contrast tooling consumes the value.
- `tests/test_obsidian_palette_contrast.py` parses `static/tokens.css` and enforces WCAG pairs — run it after any token change.
...
### 9. Obsidian workspace frame (encuadre — locked 2026-07-13)
The workspace chrome follows an Obsidian-style framed layout. Preserve it:

- **Flat sidebars**: rails AND side panel shells sit on `var(--bg-main)` with NO hard inline borders; panel bodies (`.controls`, `.detail-panel`) are `background: transparent`.
- **Floating center card**: `.center-column` is the only elevated surface — `margin: var(--workspace-gap)`, `border: 1px solid var(--border-subtle)`, `border-radius: var(--radius-workspace)`, `box-shadow: var(--shadow-xs)`, `overflow: hidden`, `background: var(--bg-panel)`. Floating menus must mount on `document.body` (overflow menu and tab picker already do) or the card clips them.
- **One shared panel header**: `.panel-header` (44px, `13px` / `var(--fw-semibold)` / `var(--text-normal)`, `padding: 0 8px 0 16px`) is IDENTICAL for the left panel and the inspector. Do not fork per-side typography again.
- **One rail selected pattern**: mora tint + edge indicator bar on BOTH rails. No per-icon colored selected variants.
- **No heavy shadows** on side panels — separation comes from the frame gap, not drop shadows.
- **Overflow safety**: `.panel-card` uses `grid-template-columns: minmax(0, 1fr)` so wide rows/segmented controls shrink instead of overflowing; `.segment-btn` ellipsizes.

## MANDATORY: post-PR review checkpoint

**After every PR that ships user-visible chrome, you MUST emit ONE of:**

1. A static HTML preview at `brain_ds/ui/design/checkpoints/<pr-slug>-preview.html` that the user can open and inspect in isolation — pulling in `_tokens.css` + `_shared.css` from the design references, NOT inlining tokens.
2. A PowerShell command to generate a live viewer from a sample graph (e.g. `uv run brain_ds ui .\tmp_sample_graph.json --output .\tmp\<pr-slug>.html`) AND a brief `brain_ds/ui/design/checkpoints/<pr-slug>-review.md` explaining what to look at and what is out of scope.
...
- New CSS variables that do not live in `brain_ds/ui/static/tokens.css`
- Hard borders or drop shadows on side panels (the frame gap separates surfaces)
- Divergent per-side panel header typography
- Skipping the post-PR review HTML

## Tests
- `tests/test_viewer.py` — chrome and a11y assertions; run after every chrome change
- `tests/test_obsidian_palette_contrast.py` — WCAG contrast over `static/tokens.css`; run after any token change
