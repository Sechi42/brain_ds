# Spec: ui-workspace-shell — Workspace Shell Layout
...
This spec defines the 5-column workspace shell that replaces the previous 3-column flat grid in `graph_viewer.html`. The shell consumes `RENDER_CONTEXT` v1.0.0 from `backend-ui-contract` without evolving it. All requirements are grouped under a single new domain: `ui-workspace-shell`.

---

## Domain: ui-workspace-shell
...
Left rail MUST be a `data-rail-side="left"` container with exactly 5 `.rail-icon` buttons (search, filters, tree, hierarchy, layout), `role="tablist"`, `aria-selected` on active, ArrowUp/ArrowDown keyboard nav. Click MUST call existing `window.brainDsUI.*.mount()` / `.unmount()`.
